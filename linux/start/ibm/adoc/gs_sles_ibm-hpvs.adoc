...
* Build a trusted container image using Hyper Protect Secure Build
* Publish trusted container image to a trusted registry
* <<Define the contract>>
...
You will use the https://cloud.ibm.com/docs/vpc?topic=vpc-financial-transaction-confidential-computing-on-hyper-protect-virtual-server-for-vpc[PayNow Node.js application], which has been built on a SLE BCI container image and published to a container registry.
...
// An additional document will describe the steps to securely build and publish the PayNow Node.js application using SLE Base Container Images.
// Mike: If the steps can be outlined, it might be useful to include here.
// Terry: I believe this would cause the document to go beyond a getting started guide but I will ask others for their opinion
...
services:
  paynow:
    image: ghcr.io/mfriesenegger/slebci-paynow-website@sha256:90e28fdd0a7415d2db5942ddb1a54b7be0ddb79fd58ee50f02d0771226a77a42
    ports:
      - "8080:8080"
      - "8443:8443"
----
