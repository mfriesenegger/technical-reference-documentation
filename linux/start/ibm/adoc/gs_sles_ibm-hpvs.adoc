...
.. Create the `vendora-data` file in the same directory.
+
For example:
+
[source, console]
...
.. Copy the `slebci-paynow-website-contract.yml` contract file from the local system to the `/var/lib/libvirt/images/slebci-paynow-website/user-data` directory on the KVM host.
...
Containerized confidential computing enalbes you to protect your workload data no matter where it is running.
